...
| IVY-007 | Work from an old run or replaced operation cannot mutate successor state. |
| IVY-008 | Frames, fields, connections, pending work, routing, hints, and routes are bounded. |
...
| IVY-015 | Public discovery uses authenticated Kademlia responses and admitted addresses; there is no PEX. |
| IVY-016 | Relay uses configured carriers while preserving end-to-end endpoint identity. |
